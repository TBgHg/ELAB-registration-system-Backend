...
func GetSpaceMemberList(ctx *gin.Context, spaceId string) (*[]Member, error) {
	...
	var members []Member
	err := svc.MySQL.WithContext(ctx).Model(&Member{}).Where(&Member{SpaceId: spaceId}).Find(&members).Error
	if err != nil {
		return nil, err
	}
...
//	db := svc.MySQL.Model(&space.Space{}).InnerJoins("members", "members.space_id = space.space_id").Where(&Member{OpenId: openid})
//	if !userMatch {
